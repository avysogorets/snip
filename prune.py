...
class Pruner(object):
  ...
  def prune_random_layer(self,model,tensors,sparsity):
    masks=[np.ones(model.layers[layer].get_weights()[0].shape) for layer in tensors]
    inds=[np.random.choice(range(len(mask.reshape(-1))),size=int(sparsity*len(mask.reshape(-1))),replace=False) for mask in masks]
